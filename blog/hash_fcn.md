...
@@graybox
A family of hash functions $\mathcal H=\{h:U\rightarrow V\}$ is called __*universal*__ if, for any two distinct elements $x,y\in U$, a randomly chosen hash function $h\in\mathcal H$ maps $x$ and $y$ to the same value with probability at most $\frac{1}{|V|}$. Formally, $\mathcal H$ is universal if:
...
We focus on families $\mathcal H$ whose range is the set of integers $V=\{0,1,\dots,M-1\}$. Constructing a universal hash function from such a family is straightforward. Here's one way to do it:
...
This $h$ is universal, with collision probability $1/M$. You can find a proof of this, e.g., in [this lecture](https://www.cs.cmu.edu/~avrim/451f11/lectures/lect1004.pdf) by Avrim Blum.
...
Here's some simple Julia code to construct the universal hash function $h$:
